...
class Posts extends \Magento\Backend\Block\Widget\Grid\Extended
{
    /**
     * Core registry
     *
    ...
    protected function _addColumnFilterToCollection($column)
    {
        ...
        if ($column->getId() == 'in_products') {
            ...
            if ($column->getFilter()->getValue()) {
                $this->getCollection()->addFieldToFilter('main_table.post_id', ['in' => $productIds]);
            } else {
                if ($productIds) {
                    $this->getCollection()->addFieldToFilter('main_table.post_id', ['nin' => $productIds]);
                }
            }
        } else {
    ...
    protected function _prepareCollection()
    {
        ...
        if ($this->isReadonly()) {
            ...
            $collection->addFieldToFilter('main_table.post_id', ['in' => $productIds]);
        }
    ...
    public function getGridUrl()
    {
        ...
        return $this->_getData(
            ...
            'vesblog/*/postGrid/category_id/'.$category->getCategoryId(),
            ['_current' => true]
            );
        }
        ...
        public function getSelectedPosts()
        {
            ...
            if($category){
                ...
                if(!empty($posts)){
                    foreach ($this->_coreRegistry->registry('current_category')->getPosts() as $_post) {
                        $products[$_post['post_id']] = ['position' => $_post['position']];
                    }
                }
            }
            return $products;
